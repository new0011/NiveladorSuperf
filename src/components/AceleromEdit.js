...
export default function AceleromEdit({ navigation }) {
    const route = useRoute();

    const {id, Superficie, Valor} = route.params;
    ...
    const handleSubmit = async (values) => {
        try {
            console.log(id)
            const response = await axios.put(`http://192.168.1.71:9000/update-datos/${id}`, values);
    ...
    return (
        <View style={styles.container}>
            <View style={styles.container1}>
                <Formik
                    initialValues={{ nombreS: Superficie, valNivel: '' }}
            ...
            <Button
                ...
                onPress={() => navigation.navigate('AceleromCons', { updatedData: route.params })}
            />
